...
// ── Webhook delivery metrics (Issue #170) ────────────────────────────────────
...
impl WebhookMetrics {
    ...
    pub fn record_latency_ms(&self, ms: u64) {
        ...
        for (i, &bound) in LATENCY_BUCKETS_MS.iter().enumerate() {
            if ms <= bound { self.inner.latency_buckets[i].fetch_add(1, Ordering::Relaxed); }
        }
...
// ── Background-task health gauges (Issue #171) ───────────────────────────────

/// Tracks how many background worker tasks are currently alive and records
/// cumulative unexpected exits (panics / unrecoverable errors).
#[derive(Clone)]
...
struct TaskHealthInner {
    /// Number of tasks currently running (signed so a double-decrement is
    /// detectable as a negative value rather than a silent wrap-around).
    healthy: AtomicI64,
    /// Cumulative count of tasks that exited unexpectedly.
    failures: AtomicU64,
}
...
impl TaskHealth {
    pub fn new() -> Self {
        Self {
            inner: Arc::new(TaskHealthInner {
                healthy: AtomicI64::new(0),
                failures: AtomicU64::new(0),
            }),
        }
    }

    /// Call once immediately before spawning a task.
    pub fn task_started(&self) {
        self.inner.healthy.fetch_add(1, Ordering::Relaxed);
    }

    /// Call at the end of a task's async block when it exits via the shutdown
    /// signal (clean exit).
    pub fn task_stopped(&self) {
        self.inner.healthy.fetch_sub(1, Ordering::Relaxed);
    }

    /// Call when a task exits due to a panic or unrecoverable error. Decrements
    /// the healthy gauge AND increments the failure counter so both signals fire.
